...
GIFDIR = os.path.join(BASE_DIR, "gifs")


# ---------------------------------------------------------------------
# Core coefficient ODEs and helpers (copied so this file is self-contained)
# ---------------------------------------------------------------------
...
def compute_loading_term_structures(params, tau_grid):
    ...
    for i, tau in enumerate(tau_grid):
        ...
        _, b_q_h[i], b_q_g[i], b_q_f[i] = get_quanto_defaultable_coeffs(
            params, float(tau)
        )
...
# ---------------------------------------------------------------------
# Low-level helper: render a single plot to an RGB array
# ---------------------------------------------------------------------


def _render_frame(plot_func, params, title_suffix=None):
    fig, ax = plt.subplots()
    plot_func(ax, params)
...
def _linspace_param_values(start, end, n_frames):
    if n_frames <= 1:
...
def _set_param(params, name, value):
    if not hasattr(params, name):
        raise AttributeError(f"DisasterModelParams has no attribute '{name}'")
    setattr(params, name, value)


def _make_gif_for_plot(
    plot_key,
    plot_func,
    base_params,
    param_name,
    ...
    _ensure_gifdir()
    values = _linspace_param_values(start, end, n_frames)
    frames = []

    # We just keep mutating the same params object; we overwrite param_name every time.
    p = base_params if base_params is not None else DisasterModelParams()

    for val in values:
        _set_param(p, param_name, float(val))
        frame = _render_frame(
            plot_func,
            p,
            title_suffix=f"{param_name}={val:.4f}",
        )
        frames.append(frame)

    duration = 1.0 / fps
    fname = f"{plot_key}_{param_name}_sweep.gif"
    outpath = os.path.join(GIFDIR, fname)
    imageio.mimsave(outpath, frames, duration=duration)
    return outpath


# ---------------------------------------------------------------------
# Plot definitions (same 13 figures, but drawn into an Axes)
# ---------------------------------------------------------------------


def _plot_bond_price_term_structures(ax, params):
    ...
    ax.plot(tau_grid, B_star, label=r"$B^*(\tau)$ (foreign risk-free)")
    ax.plot(tau_grid, B_dom, label=r"$B(\tau)$ (domestic risk-free)")
    ax.plot(tau_grid, B_D_star, linestyle="--", label=r"$B_D^*(\tau)$ (foreign def.)")
    ax.plot(
        tau_grid,
        B_quanto,
        linestyle="--",
        label=r"$\tilde B_D^*(\tau)$ (quanto)",
    )
    ax.set_xlabel(r"Maturity $\tau$ (years)")
    ax.set_ylabel(r"Price $B(\tau)$")
    ax.set_title(r"Bond price term structures")
    ax.grid(alpha=0.3)
    ax.legend()


def _plot_riskfree_loadings(ax, params):
    tau_grid = np.linspace(0.25, 10.0, 120)
    coeffs = compute_loading_term_structures(params, tau_grid)

    ax.plot(tau_grid, coeffs["b_star"], linestyle="-", label=r"$b^*(\tau)$")
    ax.plot(tau_grid, coeffs["b_dom"], linestyle="--", label=r"$b(\tau)$")
    ax.set_xlabel(r"Maturity $\tau$")
    ax.set_ylabel(r"Loading on $\lambda$")
    ax.set_title(r"Risk-free loadings $b^*(\tau)$ and $b(\tau)$")
    ax.grid(alpha=0.3)
    ax.legend()


def _plot_defaultable_loadings(ax, params):
    tau_grid = np.linspace(0.25, 10.0, 120)
    coeffs = compute_loading_term_structures(params, tau_grid)

    ax.plot(tau_grid, coeffs["b_D_f"], label=r"$b_{D,f}^*(\tau)$")
    ax.plot(tau_grid, coeffs["b_D_g"], label=r"$b_{D,g}^*(\tau)$")
    ax.set_xlabel(r"Maturity $\tau$")
    ax.set_ylabel(r"Loading on $\lambda_t^f,\lambda_t^g$")
    ax.set_title(r"Defaultable bond loadings $b_{D,f}^*(\tau)$ and $b_{D,g}^*(\tau)$")
    ax.grid(alpha=0.3)
    ax.legend()


def _plot_quanto_loadings(ax, params):
    tau_grid = np.linspace(0.25, 10.0, 120)
    coeffs = compute_loading_term_structures(params, tau_grid)

    ax.plot(tau_grid, coeffs["b_q_h"], label=r"$\tilde b_h(\tau)$")
    ax.plot(tau_grid, coeffs["b_q_g"], label=r"$\tilde b_g(\tau)$")
    ax.plot(tau_grid, coeffs["b_q_f"], label=r"$\tilde b_f(\tau)$")
    ax.set_xlabel(r"Maturity $\tau$")
    ax.set_ylabel(r"Loading on $\lambda_t^h,\lambda_t^g,\lambda_t^f$")
    ax.set_title(r"Quanto loadings $\tilde b_i(\tau)$")
    ax.grid(alpha=0.3)
    ax.legend()


def _plot_lambda_f_loadings_comparison(ax, params):
    tau_grid = np.linspace(0.25, 10.0, 120)
    coeffs = compute_loading_term_structures(params, tau_grid)
    ...
    ax.plot(tau_grid, coeffs["b_D_f"], label=r"$b_{D,f}^*(\tau)$")
    ax.set_xlabel(r"Maturity $\tau$")
    ax.set_ylabel(r"Loading on $\lambda_t^f$")
    ax.set_title(
        r"Loadings on $\lambda_t^f$: quanto $\tilde b_f(\tau)$ vs. defaultable $b_{D,f}^*(\tau)$"
    )
    ax.grid(alpha=0.3)
    ax.legend()


def _plot_yield_term_structures(ax, params):
    tau_grid = np.linspace(0.5, 10.0, 120)
    lam_h, lam_g, lam_f = params.lam_bar_h, params.lam_bar_g, params.lam_bar_f

    curves = compute_yield_term_structures(params, tau_grid, lam_h, lam_g, lam_f)

    ax.plot(tau_grid, curves["y_star"], label=r"$y^*(\tau)$ (foreign RF)")
    ax.plot(tau_grid, curves["y_dom"], label=r"$y(\tau)$ (domestic RF)")
    ax.plot(tau_grid, curves["y_D_star"], label=r"$y_D^*(\tau)$ (foreign def.)")
    ax.plot(tau_grid, curves["y_quanto"], label=r"$\tilde y(\tau)$ (quanto)")
    ax.set_xlabel(r"Maturity $\tau$")
    ax.set_ylabel(r"Yield (annual \%)")
    ax.set_title(r"Yield term structures")
    ax.grid(alpha=0.3)
    ax.legend()


def _plot_spread_term_structures(ax, params):
    tau_grid = np.linspace(0.5, 10.0, 120)
    lam_h, lam_g, lam_f = params.lam_bar_h, params.lam_bar_g, params.lam_bar_f

    curves = compute_yield_term_structures(params, tau_grid, lam_h, lam_g, lam_f)

    ax.plot(tau_grid, curves["s_star"], label=r"$s^*(\tau)$ (foreign spread)")
    ax.plot(tau_grid, curves["s_tilde"], label=r"$\tilde s(\tau)$ (domestic spread)")
    ax.plot(tau_grid, curves["q_tilde"], label=r"$\tilde q(\tau)$ (quanto basis)")
    ax.axhline(0.0, color="black", linewidth=0.7, alpha=0.7)
    ax.set_xlabel(r"Maturity $\tau$")
    ax.set_ylabel(r"Spread (percentage points)")
    ax.set_title(r"Credit spreads and quanto basis")
    ax.grid(alpha=0.3)
    ax.legend()


def _plot_sensitivity_lambda_f(ax, params):
    tau_grid = np.linspace(0.5, 10.0, 120)
    lam_h, lam_g, lam_f = params.lam_bar_h, params.lam_bar_g, params.lam_bar_f

    dBD_dlamf, dBq_dlamf, _, _ = compute_sensitivities_vs_tau(
        params, tau_grid, lam_h, lam_g, lam_f
    )

    ax.plot(tau_grid, dBD_dlamf, label=r"$\partial B_D^*(\tau)/\partial \lambda_t^f$")
    ax.plot(
        tau_grid,
        dBq_dlamf,
    ...
    ax.set_xlabel(r"Maturity $\tau$")
    ax.set_ylabel(r"Sensitivity to $\lambda_t^f$")
    ax.set_title(r"Sensitivity of foreign and quanto bonds to $\lambda_t^f$")
    ax.grid(alpha=0.3)
    ax.legend()


def _plot_sensitivity_quanto_all_intensities(ax, params):
    tau_grid = np.linspace(0.5, 10.0, 120)
    lam_h, lam_g, lam_f = params.lam_bar_h, params.lam_bar_g, params.lam_bar_f

    _, dBq_dlamf, dBq_dlamg, dBq_dlamh = compute_sensitivities_vs_tau(
        params, tau_grid, lam_h, lam_g, lam_f
    )

    ax.plot(tau_grid, dBq_dlamf, label=r"$\partial \tilde B_D^*/\partial \lambda_t^f$")
    ax.plot(tau_grid, dBq_dlamg, label=r"$\partial \tilde B_D^*/\partial \lambda_t^g$")
    ax.plot(tau_grid, dBq_dlamh, label=r"$\partial \tilde B_D^*/\partial \lambda_t^h$")
    ax.set_xlabel(r"Maturity $\tau$")
    ax.set_ylabel(r"Sensitivity of $\tilde B_D^*$")
    ax.set_title(
        r"Quanto bond sensitivities to $(\lambda_t^h,\lambda_t^g,\lambda_t^f)$"
    )
    ax.grid(alpha=0.3)
    ax.legend()


def _plot_quanto_basis_vs_tau(ax, params):
    lam_h, lam_g, lam_f = params.lam_bar_h, params.lam_bar_g, params.lam_bar_f
    ...
    ax.plot(tau_grid, q_tilde * 100, label=r"$\tilde q(\tau)$")
    ax.set_xlabel(r"Maturity $\tau$")
    ax.set_ylabel(r"Quanto basis (bps)")
    ax.set_title(r"Quanto basis term structure $\tilde q(\tau)$")
    ax.grid(alpha=0.3)
    ax.legend()


def _plot_quanto_basis_vs_lambda_f(ax, params):
    ...
    ax.plot(lam_f_grid, q_vals * 100)
    ax.set_xlabel(r"Foreign disaster intensity $\lambda_t^f$")
    ax.set_ylabel(r"Quanto basis at $\tau_0$ (bps)")
    ax.set_title(
        r"Quanto basis $\tilde q(\tau_0)$ vs $\lambda_t^f$,  " r"$\tau_0=5$ years"
    )
    ax.grid(alpha=0.3)


def _plot_corr_vs_lambda_f(ax, params):
    lam_h, lam_g = params.lam_bar_h, params.lam_bar_g
    ...
    ax.plot(lam_f_vals, rho_P_vals, label=r"$\rho^{\mathbb{P}}$")
    ax.plot(lam_f_vals, rho_Q_vals, label=r"$\rho^{\mathbb{Q}}$")
    ax.plot(lam_f_vals, rho_Qs_vals, label=r"$\rho^{\mathbb{Q}^*}$")
    ax.axhline(0.0, color="black", linewidth=0.7, alpha=0.6)
    ax.set_xlabel(r"Foreign disaster intensity $\lambda_t^f$")
    ax.set_ylabel(r"Instantaneous corr. $\rho^{\mathbb{M}}(d\ln e_t, dh_t^*)$")
    ax.set_ylim(0.0, 1.0)
    ax.set_title(
...
def _plot_corr_vs_lambda_g(ax, params):
    ...
    rho_Qs_vals = np.array(
        [_rho_Q_star(params, lam_h, lam_f, lg) for lg in lam_g_vals]
    )

    ax.plot(lam_g_vals, rho_P_vals, label=r"$\rho^{\mathbb{P}}$")
    ax.plot(lam_g_vals, rho_Q_vals, label=r"$\rho^{\mathbb{Q}}$")
    ax.plot(lam_g_vals, rho_Qs_vals, label=r"$\rho^{\mathbb{Q}^*}$")
    ax.axhline(0.0, color="black", linewidth=0.7, alpha=0.6)
    ax.set_xlabel(r"Global disaster intensity $\lambda_t^g$")
    ax.set_ylabel(r"Instantaneous corr. $\rho^{\mathbb{M}}(d\ln e_t, dh_t^*)$")
    ax.set_ylim(0.0, 1.0)
    ax.set_title(
...
PLOT_REGISTRY = {
    # 3.1 Bond prices
    "sec6_1_bond_price_term_structures": _plot_bond_price_term_structures,
    # 3.2 Loadings (4 plots)
    "sec6_2a_riskfree_loadings": _plot_riskfree_loadings,
    "sec6_2b_defaultable_loadings": _plot_defaultable_loadings,
    "sec6_2c_quanto_loadings": _plot_quanto_loadings,
    "sec6_2d_defaultable_vs_quanto_loadings": _plot_lambda_f_loadings_comparison,
    # 3.3 Yields and spreads
    "sec6_3_yield_term_structures": _plot_yield_term_structures,
    "sec6_3_spread_term_structures": _plot_spread_term_structures,
    # 3.4 Sensitivities (2 plots)
    "sec6_4_sensitivity_lambda_f": _plot_sensitivity_lambda_f,
    "sec6_4_sensitivity_quanto_all_intensities": _plot_sensitivity_quanto_all_intensities,
    # 3.5 Quanto basis (2 plots)
    "sec6_5_quanto_basis_vs_tau": _plot_quanto_basis_vs_tau,
    "sec6_5_quanto_basis_vs_lambda_f": _plot_quanto_basis_vs_lambda_f,
    # Section 3 correlations (2 plots)
    "sec3_corr_vs_lambdaf": _plot_corr_vs_lambda_f,
    "sec3_corr_vs_lambdag": _plot_corr_vs_lambda_g,
}
...
def make_gif_for_single_plot(
    ...
    """
    Create a GIF for a single figure while sweeping one parameter.

    Parameters
    ----------
    plot_key : str
        One of the keys in PLOT_REGISTRY, e.g. 'sec6_3_spread_term_structures'.
    param_name : str
        Name of the DisasterModelParams attribute to vary (e.g. 'eta1').
    start, end : float
        Start and end values of the sweep.
    n_frames : int, optional
        Number of frames in the GIF.
    fps : int, optional
        Frames per second in the resulting GIF.
    base_params : DisasterModelParams, optional
        If None, a fresh DisasterModelParams() is created.
    """
    if plot_key not in PLOT_REGISTRY:
        raise ValueError(
            f"Unknown plot_key '{plot_key}'. Valid keys: {list(PLOT_REGISTRY.keys())}"
        )

    if base_params is None:
        base_params = DisasterModelParams()

    plot_func = PLOT_REGISTRY[plot_key]
    return _make_gif_for_plot(
        plot_key=plot_key,
        plot_func=plot_func,
...
def make_all_gifs_for_parameter(
    param_name,
    start,
    end,
    n_frames=30,
    ...
    """
    Sweep a single DisasterModelParams parameter and create GIFs
    for all 13 plots defined in this file.

    Example
    -------
    >>> make_all_gifs_for_parameter("eta1", 0.05, 0.20, n_frames=40)
    """
    if base_params is None:
        base_params = DisasterModelParams()

    outputs = {}
    for key, plot_func in PLOT_REGISTRY.items():
        outpath = _make_gif_for_plot(
            plot_key=key,
            plot_func=plot_func,
            base_params=base_params,
            param_name=param_name,
...
def main():
    ...
    param_name = "lam_bar_f"
    start = 0.0001
    end = 0.005
    n_frames = 100
    fps = 20

    make_all_gifs_for_parameter(
        param_name=param_name,
        start=start,
        end=end,
        n_frames=n_frames,
